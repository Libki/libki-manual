...
Using a MySQL or MariaDB container on the same Docker network is considered good practice.
...
So you'll want to set up the database container, then start the Libki container with the following environment variables set:
LIBKI_INSTANCE=
LIBKI_DB_DSN=dbi:mysql:$DB_NAME;host=$DB_HOSTNAME;port=3306
LIBKI_DB_USER=$DB_USERNAME
LIBKI_DB_PASSWORD=$DB_PASSWORD
...
LIBKI_DB_PORT=3306
LIBKI_DB_DATABASE=$DB_NAME
LIBKI_TZ=America/Denver # Set to your timezone
