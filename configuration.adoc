...
** This choice limits a time extension in the case that the user's client is reserved to not overlap with that reservation. Reservations for other client's are not taken into account. The Reservation Gap length taken into account.
...
===== Self-registered guest passes

This setting controls whether clients will have a 'Log in as guest' button that automatically creates a guest account for them on login.  Users will receive a one-time message on the Libki Client with their username and password, allowing them to lock their session or utilize print release services.

TIP: the Libki Client will need to be rebooted at least once before this option appears.

===== Expire remaining guest pass time on logout

If enabled, this setting will zero out any remaining time on a guest account after that account logs out of a client.  This is useful for preventing unauthorized users from picking up discarded guest passes and getting extra time beyond what is allowed by library policy.
...
<SIP>
    ...
    deny_on_field AF:Your library card has expired.:EXPIRE # <7>
    category_field PC # <8>
    birthdate_field PB # <9>
    pattern_personal_name , # <10>
    pattern_personal_name_reverse 1 <11>
    ILS Koha # <12>
    skip_import_personal_name 0 <13>
    hold_notification 0 <14>
</SIP>
----
...
<7> With the format _<Field>:<Message>:<Value>_ the login is denied if the field contains _<Value>_ and _<Message>_ is shown to the patron.
<8> Specify a SIP tag here that contains the user category in the SIP user lookup response
<9> Specify a SIP tag here that contains the user's date of birth  in the SIP user lookup response, default is 'PB'.  Some ILS, like Atriuum by Booksystems, use 'PD'.
<10> Specify a pattern for splitting lastname and firstname in personal name field(AE) in SIP response
<11> If the patrons last name and first name are reversed, this option will correct the order
<12> Can be 'Koha' or 'Evergreen' at this time, controls how waiting holds are calculated for alerting patrons
<13> If set, patron names will not be imported into Libki, only the cardnumber/userid and par
<14> Disables hold notification if set to 0
...
<LDAP>
    enable 1
    ldaps 0
    host ldap://ldap.mylibrary.org
    port 368
    adminDn admin_dn
    adminPwd admin_password
    searchBase OU=...,DC=mylibrary,DC=org
...
This section manages integration with electronic payment gateways.

===== Stripe Payments

Stripe integration requires a https://stripe.com[Stripe.com] account.  You'll need to retrieve 3 values from the Dashboard:

. *Stripe Publishable Key*: found under Dashboard -> Developers -> API Keys
. *Stripe Secret Key*: found under Dashboard -> Developers -> API Keys
. *Stripe Webhook Signing Secret*: Requires adding an 'Event Destination' within Stripe Workbench.
.. Go to the 'Webhooks' tab
.. Click the 'Add destination' button (upper right)
.. Enter the Endpoint URL: *<your Libki URL>*/api/v2/transactions/stripe/webhook 
.. Give the destination a name and description
.. Select the *_payment_intent.payment_failed_* and *_payment_intent.succeeded events_* from the list.
.. Click the 'Save destination' button

You can test your Stripe integration in sandbox mode before switching to a live account.  A list of testing credit card numbers can be found at https://docs.stripe.com/testing?testing-method=card-numbers#cards.
...
===== Queued Print jobs timeout

The amount of time a print job can be in the Queued status before the job is set to Expired, and a new Pending job is added. This is meant to allow print jobs taken by a Print Manager that has become unresponsive to be re-printed automatically.
Leave this setting empty to disable the feature.

===== Allow administrators to view user print job contents

Whether to allow admins the ability to see the content of a user's print job.

===== Gratis Printing

This feature grants the ability to give a user either free pages of printing or a dollar 'discount' on print jobs that can reset on a daily, weekly or monthly basis.
This gratis allowance is used first when releasing a print job, with any remaining balance being assessed against the users funds.  If sufficiently large, the free printing allowance can be used against multiple print jobs.

Default values are set here, and can vary between regular users and guests. Values can also be updated individually in the User Edit screen.
...
The amount of time in minutes before a user's existing reservation in which a login to ANY client will automatically cancel their reservation.

TIP: The value of *Reservation Auto-Cancel* should be a whole number.

===== Minimum reservation time required

The minimum allowed time a reservation can be made for.  Defaults to 1 minute if left blank.

===== Reservation Gap

The number of minutes between reservations, to allow for physical transitions times.  Defaults to 0 (no gap).

===== Display Public Reservations Page

Whether to display the list of current clients and their reservation status on the public Libki page.  Defaults to enabled.

===== Display the status of the first reservation within

Time range in which to show reservations on the public page; defaults to 60 minutes from now.

===== Opening time for reservation

Universal time of day when PCs are available for reservation

===== Display usernames

This setting determines if a Libki client that is reserved and waiting will display the username of the person it is waiting for.  Can be:
* Username
* First and last name
* Anonymous
* Disabled
...
Whether to display user's names, and allow for the entry of this data.
...
API keys are required for Libki Print Manager and Libki Print Station to connect to Libki Server.
On this page you can create new keys and delete existing ones.

Keys have a human-readable note (to indicate their purpose) and a Type (PrintManager or PrintStation).  The key value itself is a UUID.
...
* auto_release: Optional
** if set to "yes", prints to this printer will not be held for release, but instead be released automatically if funds are available.
** if set to "client", prints will be auto-released only if they come from Libki Clients (held for release via web upload)
** if set to "public", prints will be auto-released only if they are uploaded to the web (held for release from Libki Clients)
...
Libki determines the desired copies for a print by looking at the filename of the saved PDF.  If it ends in the pattern {number}_{number}.pdf, the first number is interpreted as the copies parameter.
...
[NOTE]
===============================
As of Libki 26.02, printer configuration can be handled through the Client installer wizard.
This depends on using ClawPDF as the print driver; this must be installed first.
When prompted by the Client installer wizard, enter the printer names you wish to enable for the client, one per line, with the first line being the default printer.
The wizard will take care of creating the printer directories, updating the Client .ini file, and configuring ClawPDF
===============================
