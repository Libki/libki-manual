...
<SIP>
    ...
    pattern_personal_name_reverse 1 <9>
    ILS Koha # <10>
    skip_import_personal_name 0 <11>
    hold_notification 0 <12>
</SIP>
----
...
<9> If the patrons last name and first name are reversed, this option will correct the order
<10> Can be 'Koha' or 'Evergreen' at this time, controls how waiting holds are calculated for alerting patrons
<11> If set, patron names will not be imported into Libki, only the cardnumber/userid and par
<12> Disables hold notification if set to 0
