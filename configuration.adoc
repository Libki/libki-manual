...
If there is no catch-all rule like this, Libki will fall back to the origin default time allowance settings
...
would apply to any user of category MyCategory1 *or* MyCategory2 logging into a client at locatin MyLocationA.

This is, this rule will apply to all the value in the list for that criteria.

Another way to think of it is that all creteria are AND'ed, and all the value for a criteria are OR'ed.
